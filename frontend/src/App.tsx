...
//               {/* FIX: AJUSTADO PARA EVITAR CONCATENAÇÃO EM DUPLICIDADE */}
...
function App() {
  ...
  async function loadNotificacoes() {
    try {
      // SIMULAÇÃO: Estrutura idêntica à que virá futuramente do seu Python
  ...
  // Cronômetro (Polling) em segundo plano ativado após o Login
  useEffect(() => {
    if (user) {
      loadNotificacoes(); // Busca inicial imediata

      // Checa por novidades a cada 30 segundos de forma automatizada
      const interval = setInterval(() => {
        loadNotificacoes();
      }, 30000);

      return () => clearInterval(interval); // Limpa o timer se deslogar
    }
  }, [user])
